...
@implementation WProgressHUD

#pragma mark 加载中动画
+ (instancetype)showHUDShowText:(NSString *)showText {
    WProgressHUD *hud = [[self alloc] initWithFrame:[UIScreen mainScreen].bounds showText:showText HUDType:WHUDLoadingType];
...
+ (instancetype)showSuccessfulAnimatedText:(NSString *)ShowText {
    ...
    WProgressHUD *hud = [[self alloc] initWithFrame:[UIScreen mainScreen].bounds showText:ShowText HUDType:WHUDSuccessfulAnimatedType];
...
+ (instancetype)showErrorAnimatedText:(NSString *)ShowText {
    [WProgressHUD hideAllHUDAnimated:NO];
    WProgressHUD *hud = [[self alloc] initWithFrame:[UIScreen mainScreen].bounds showText:ShowText HUDType:WHUDErrorAnimatedType];
    [hud show:YES view:hud.toast];
    [[UIApplication sharedApplication].keyWindow addSubview:hud];
    [self performSelector:@selector(DelayHideHud) withObject:nil afterDelay:1.5];
    return hud;
...
+ (instancetype)showDpromptText:(NSString *)showText {
    [WProgressHUD hideAllHUDAnimated:NO];
    WProgressHUD *hud = [[self alloc] initWithFrame:[UIScreen mainScreen].bounds showText:showText HUDType:WHUDpromptTextType];
...
+ (NSUInteger)hideAllHUDAnimated:(BOOL)animated {
    ...
    for (UIView *aView in subviews) {
        if ([aView isKindOfClass:[WProgressHUD class]]) {
            [huds addObject:aView];
        }
    }
    
    for (WProgressHUD * hud in huds) {
        if (hud.type == WHUDpromptTextType) {
...
- (instancetype)initWithFrame:(CGRect)frame showText:(NSString *)showText HUDType:(WHUDType)type {
    ...
    if (self) {
        ...
        CGSize size = [WPublicManager workOutSizeWithStr:showText andFont:14 value:[NSValue valueWithCGSize:CGSizeMake(self.frame.size.width - 30, 999)]];
        if (size.width > 100) {
            _toast.width = size.width + 30;
        }
        _toast.center=CGPointMake(APP_WIDTH / 2, APP_HEIGHT / 2);
        _toast.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.8];
        _toast.layer.cornerRadius = 10;
        _toast.hidden = YES;
        [self addSubview:_toast];
        
        if (type==WHUDLoadingType) {
            //加载
            [self loadingAnimated];
        } else if (type==WHUDSuccessfulAnimatedType) {
            //成功
            [self pictureCorrect];
        } else if (type==WHUDErrorAnimatedType) {
            //失败效果
            [self picturError];
        } else if (type==WHUDpromptTextType) {
            //提示文字
            [self DpromptText];
        }
...
- (void)DpromptText {
    ...
    CGSize size = [WPublicManager workOutSizeWithStr:_tipText andFont:14 value:[NSValue valueWithCGSize:CGSizeMake(self.frame.size.width-40, 999)]];
    if (size.width > 100) {
